...
{
    /// <summary>
    /// API Controller for product import operations
    /// </summary>
    [ApiController]
    ...
    public class ProductImportController : ControllerBase
    {
        ...
        /// <summary>
        /// Preview product import data from Excel file
        /// </summary>
            ...
            [FromForm] bool validateData = true)
        {
            try
            {
                ...
                _logger.LogInformation("Product preview import request received. File: {FileName}, Size: {FileSize}", 
            ...
            {
                _logger.LogError(ex, "Error during product import preview");
            ...
            [FromForm] bool validateData = true)
        {
            try
            {
                ...
                _logger.LogInformation("Product import request received. File: {FileName}, Size: {FileSize}, UserId: {UserId}", 
                    file.FileName, file.Length, userId);

                var result = await _importService.ImportProductsAsync(file, validateData, userId);
                ...
                var response = new
                {
                    ...
                    summary = new
                    {
                        successfulProducts = result.SuccessfulProducts.Take(10).Select(p => new
                        {
                        ...
                        failedProducts = result.FailedProducts.Take(10).Select(p => new
                        {
                            rowNumber = p.RowNumber,
                            code = p.Code,
                            productName = p.ProductName,
